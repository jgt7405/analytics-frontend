...
3. `src/hooks/use<Thing>.ts`: `useQuery` with `queryKey: ["<resource>", ...params]`, `enabled` guard, optional `initialData`, and `...queryCachePolicy("<class>")` from `@/lib/cache-policy`. Pick the class from `docs/decisions/cache-classes.md`. If the new path isn't `/history`, live games or a team list, `cacheClassForBackendPath()` treats it as `currentStandings` for the CDN and server fetches; add a pattern there (with a test in `src/lib/__tests__/cache-policy.test.ts`) if it belongs elsewhere.
