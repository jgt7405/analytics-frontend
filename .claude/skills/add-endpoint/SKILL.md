...
Then: response types in `src/types/`, and an expected-URL case in `src/services/__tests__/api.integration.test.ts`. The contract tests and `urls.test.ts` pick up the new entry automatically.
...
- Note the trailing slash: without it you get a 308 redirect first. In code, always build the URL with `apiUrl("<key>", params, query)` / `apiPath(...)` (`src/api/urls.ts`); ESLint rejects anything else, and the smoke tests fail on redirected proxy calls.
