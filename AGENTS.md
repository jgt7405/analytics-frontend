...
| `npm run build` / `npm start` | Production build (webpack, see below) / serve it |
...
- `public/sw.js` and `public/workbox-*.js` are generated by the build. Never edit or commit them.
- `npm run build` uses `next build --webpack` because the PWA plugin (`next-pwa`) is webpack-only. `npm run dev` uses Turbopack (the Next 16 default). Don't add a custom `webpack` config for dev; the PWA replacement (plan step 5) will move the build to Turbopack too.
