...
- `trailingSlash: true`: internal links should end in `/`. Build every backend URL from the endpoint list: `apiUrl("basketball.twv", { conference }, { season })` or `apiPath(...)` from `src/api/urls.ts` in the browser, `fetchEndpoint(...)` in `src/lib/server-api.ts` on the server. Never by hand (ESLint enforces it): the helpers check the key and parameters and add the trailing slash, without which every visitor pays a 308 redirect round trip (the smoke tests fail on it).
