...
export interface CWVTableProps {
  cwvData: { teams: TeamData[]; games: GameData[] };
  className?: string;
  season?: string;
}

// Shared by football and basketball so the two CWV pages can't drift apart
// in layout or tooltip; the sport only picks the team-page route.
function CWVTable({
  cwvData,
  className,
  season,
  sport,
}: CWVTableProps & { sport: "football" | "basketball" }) {
  ...
  const navigateToTeam = useCallback(
    (teamName: string) => {
      const path = season
        ? `/${sport}/${season}/team/${encodeURIComponent(teamName)}`
        : `/${sport}/team/${encodeURIComponent(teamName)}`;
      router.push(path);
    },
    [router, season, sport],
  );
