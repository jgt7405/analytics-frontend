...
const HOVER_LENS_PLUGIN: Plugin<"line"> = {
  id: "football-champ-game-hover-lens",
...
export default function FootballChampGameHistoryChart({
  ...
  useEffect(() => {
    return () => {
      document.getElementById("chartjs-tooltip-champgame")?.remove();
    };
  }, []);
  ...
  const range = getFootballDateRange(season, champGameData);
  const filteredChampGameData = filterDataToRange(champGameData, range);
  ...
  const allDatesFromData = [
    ...new Set(filteredChampGameData.map((d) => d.date)),
  ].sort();
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        title: {
          display: true,
          text: "Championship Game Probability (%)",
      ...
      <div className="px-3 pb-2 sm:px-4">
        <div className="relative" style={{ height: chartHeight }}>
          <Line
            ...
            role="img"
            aria-label="Championship game probability history showing every team's chance of reaching the conference championship game over time. Hover a date to see all teams ranked for that date."
          />
          ...
          {allTeamsSorted.map((team) => {
            ...
            return (
              <button
                ...
                aria-label={`${team.team_name}, final championship game probability ${Math.round(team.final_pct)}%. Select to emphasize this team.`}
                ...
                className={cn(
                  "flex min-w-0 cursor-pointer flex-col items-center gap-0.5 rounded-xl border-2 bg-white/80 px-1 pb-1.5 pt-1 transition-[border-width,opacity,filter,background-color] hover:bg-slate-50 dark:bg-slate-900/60 dark:hover:bg-slate-800",
