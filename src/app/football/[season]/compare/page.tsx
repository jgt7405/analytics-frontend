...
export default function FootballCompareArchivePage({
    ...
    async (teamName: string): Promise<TeamData | null> => {
      ...
      try {
        const response = await fetch(
          `/api/proxy/football_team/${encodeURIComponent(teamName)}?season=${season}`
        );
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const response = await fetch(
          `/api/proxy/football_teams?season=${season}`
        );
        const data = await response.json();
  ...
  const downloadChart = async () => {
    ...
    try {
      ...
      await saveCanvasImage(
        canvas,
        `football-compare-${season}-${new Date().toISOString().split("T")[0]}.png`,
