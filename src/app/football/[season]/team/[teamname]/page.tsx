...
interface ArchiveFootballTeamPageProps {
  params: {
    season: string;
    teamname: string;
  };
...
export default function ArchiveFootballTeamPage({
  ...
  // Main team data hook - with season parameter for archive mode
  const {
    data: teamData,
    isLoading: loading,
    error: queryError,
    refetch,
  } = useFootballTeam(teamname, season);
  ...
  }, [teamname, season, trackEvent]);

  // Update URL with teamConf when team data loads
  useEffect(() => {
  ...
  const navigateToTeam = (targetTeam: string) => {
    if (targetTeam && targetTeam !== teamname) {
      router.push(
        `/football/${season}/team/${encodeURIComponent(targetTeam)}`
      );
    }
  };
