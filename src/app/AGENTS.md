...
- `<sport>/[season]/<page>/page.tsx`: archive season. Thin client wrapper rendering the same Content with `season={params.season}`. `[season]/layout.tsx` sets `noindex`.
...
Gotchas: pages that read `useSearchParams` (e.g. `?conf=`) need `export const dynamic = "force-dynamic"` or a `<Suspense>` boundary; URLs end in `/`; don't route current-season pages through `[season]` (noindex).
