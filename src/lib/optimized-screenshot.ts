...
function createScreenshotWrapper(
  ...
  let actualContentWidth = table ? table.offsetWidth : clone.offsetWidth;

  // For non-table components, use measured width with minimal padding
  if (!table) {
    // Find the actual content width by checking child elements
    const contentElements = clone.querySelectorAll("[data-component-type]");
    if (contentElements.length > 0) {
      actualContentWidth = (contentElements[0] as HTMLElement).offsetWidth;
    }
  }

  const wrapperWidth = actualContentWidth + 100; // Reduced padding from 200 to 100
  ...
  wrapper.style.cssText = `
    ...
    padding: 24px 30px; /* Reduced horizontal padding */
